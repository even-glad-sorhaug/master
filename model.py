...
def build_model(system_data):
    ...
    def OBJ_cost_min(model):
        ...
        maintenance = sum(m.C_g_maint[g] * m.b_g[g] for g in m.G) +\
                      sum(m.C_h_maint[h] * m.b_h[h] for h in m.H) + m.C_pv_maint + m.C_st_maint
        ...
        return fuel_cost + import_costs + investment + maintenance + grid_power_costs +\
               dh_grid_costs - export_costs - certificate_savings
