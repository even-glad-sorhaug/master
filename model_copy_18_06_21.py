...
def build_model(system_data):
    ...
    def OBJ_cost_min(model):
        ...
        certificate_savings = sum(m.C_g_fuel[2] * m.gen[2, t] / m.P_eta[2] + m.pv[t] for t in m.T) * m.C_gc
        ...
        grid_power_costs = (m.C_grid_power*max(m.p_imp) + m.C_fixtar) * m.period * 1.25
        dh_grid_costs = (calc_dh_power_cost(m,m.period,m.start) + m.C_dh_fixed) * 1.25
...
def build_model_weekly(system_data,start):
    Demand, Prod, Base, Storage = Data_fetch(system_data)
    ...
    r = Base["r"]                   # Discount rate
    epsi = {}
    for n in RangeSet(30):              # Annuity factor
        epsi[n] = r/(1-(1+r)**(-n))
    hours = 24*days                    # Duration of period
    period = hours/8760                 # Fraction of one year
    ...
    P_dem = {}  # kWh
    Q_dem = {}  # kWh
    for n in RangeSet(start,start+hours):  #
        P_dem[n] = Demand["P_el"][n]
        Q_dem[n] = Demand["Q_sh"][n] + Demand["Q_hw"][n]
    #P_dem_max = max(P_dem.values())
    ...
    m.H = RangeSet(Q_min.__len__())
    m.B = RangeSet(P_min.__len__()+Q_min.__len__())
    m.T = RangeSet(start,start+hours)
    m.start = start
    ...
    m.C_dh_entar = Param(m.T,initialize= C_dh_entar)
    m.C_dh_power = Param(m.T,initialize=c_dh_powtar,mutable=True)
    m.C_dh_fixed = Param(initialize= C_dh_fixed)
    m.Max_dh = Param(initialize=dh_max)  # Transmission capacity of grid
    m.Fi_dh = Param(initialize= em_dh)
    ...
    def disjCh(block,t):
        ...
        block.c = Constraint(expr=model.p_bat_ch[t] == 0)
    m.d_batch = Disjunct(m.T, rule=disjCh)
    m.c_end = Constraint(expr=m.p_bat_dis[start+hours] <= (m.v_bat[start+hours] - v_bat_min))
    ...
    def disjCh(block,t):
        ...
        block.c = Constraint(expr=model.q_tes_ch[t] == 0)
    m.d_tesch = Disjunct(m.T, rule=disjCh)
    m.d_end = Constraint(expr=m.q_tes_dis[start+hours] <= (m.v_tes[start+hours] - v_tes_min))
    ...
    def OBJ_cost_min(model):
        ...
        import_costs = sum(m.p_imp[t] * m.C_imp[t] for t in m.T) + sum(m.dh[t] * m.C_dh_entar[t] for t in m.T)
        emission_costs = m.C_co2 * (sum(m.Fi_g[g] * m.gen[g, t] / m.P_eta[g] for g in m.G for t in m.T) + \
                        m.C_co2 * sum(m.Fi_h[h] * m.heat[h, t] / m.Q_eta[h] for h in m.H for t in m.T))
        grid_power_costs = (m.C_grid_power*max(m.p_imp) + m.C_fixtar) * m.period * 1.25
    #Her er må du endre tilbake til Clac_dh_power:
        dh_grid_costs = (calc_dh_power_cost(m,m.period,m.start) + m.C_dh_fixed) * 1.25
        export_costs = sum(m.p_exp[t] * m.C_exp[t] for t in m.T)
        return fuel_cost + import_costs + investment + maintenance + grid_power_costs + dh_grid_costs -\
               export_costs - certificate_savings #+ emission_costs
    m.o = Objective(rule= OBJ_cost_min, sense= minimize)

    return m

def build_model_el(system_data,spot_percentage):
    Demand, Prod, Base, Storage = Data_fetch(system_data)
    ...
    grid_max = {}                       # kW
    for n in RangeSet(hours):
        C_imp[n] = (Prod["Spot"][n]*spot_percentage + c_entar) * 1.25
        C_exp[n] = Prod["Spot"][n]*spot_percentage
        em_grid[n] = Prod["Gridemi"][n]
        grid_max[n] = Base["Grid_max"]
        C_dh_entar[n] = Prod["DH_energy"][n] * 1.25
        c_dh_powtar[n] = Prod["DH_power"][n]
...
def initiate_maintainance(m,t,gen,duration):
    if gen == 'bio':
        g = 2
    elif gen == 'fc':
        g = 1
...
def calc_total_cost(m):
    investment = sum(m.C_g_invest[g] * m.b_g[g].value for g in m.G) +\
        sum(m.C_h_invest[h] * m.b_h[h].value for h in m.H) + m.C_pv_invest + m.C_st_invest
    maintenance = sum(m.C_g_maint[g] * m.b_g[g].value for g in m.G) +\
                  sum(m.C_h_maint[h] * m.b_h[h].value for h in m.H) + m.C_pv_maint + m.C_st_maint
    generator_operation = sum(m.C_g_fuel[g].value * m.gen[g, t].value / m.P_eta[g] for g in m.G for t in m.T)
    ...
    emission_costs = m.C_co2 * (sum(m.Fi_g[g] * m.gen[g, t].value / m.P_eta[g] for g in m.G for t in m.T) +\
        m.C_co2 * sum(m.Fi_h[h] * m.heat[h, t].value / m.Q_eta[h] for h in m.H for t in m.T))
    grid_power_costs = (m.C_grid_power * max(m.p_imp[t].value for t in m.T) + m.C_fixtar) * m.period * 1.25
    dh_grid_costs = (calc_dh_power_cost(m,m.period,m.start) + m.C_dh_fixed) * 1.25
    export_costs = sum(m.p_exp[t].value * m.C_exp[t] for t in m.T)
    certificate_savings = sum(m.C_g_fuel[2].value * m.gen[2, t].value / m.P_eta[2] + m.pv[t] for t in m.T) * m.C_gc

    return fuel_cost + import_costs + investment + maintenance + grid_power_costs +\
           dh_grid_costs - export_costs - certificate_savings #+ emission_costs

def calc_dh_power_cost(m,period,start):
    short = [2]
    medium = [4,6,8,9,11]
    long = [1,3,5,7,10,12]
    if period <= 1/52:
        c_dh_power = m.C_dh_power[start] * max(m.dh[t].value for t in m.T) * 0.25
    elif period >= 1/3:
        start_day = 1
        c_dh_power = 0
        ...
        c_dh_power = m.C_dh_power[1] * max(m.dh[t].value for t in m.T)
    return c_dh_power

def calc_dh_power_cost_new(m,period,start):
    c_dh_power = 5.8 * max(m.dh[t].value for t in m.T)
    return c_dh_power
