...
def multiple_cases_strategic_fc_control(dict,Imin,fc,summary_file_name):
    # Runs simulations for multiple cases and stores the results in separate files
    system_data = dict
    #system_data = {1: "system_data.xls"}
    solver = SolverFactory('gurobi')
...
def multiple_cases_blackout(dict,blackout,fc,sf_name):
    # Runs simulations for multiple cases and stores the results in separate files
    system_data = dict
...
def sensitivity_analysis_h2(dict,Imin,fixed,summary_file_name,inc):
    ...
    for key in system_data.keys():
        ...
        for h in range(1,30):
            hp += inc
            h_suffix = "_sa_h2_" + str(round(hp,1))
                ...
                for j in range (1,3-fixed):
                    if j == 1:
                        ...
                        if fixed:
                            ...
                            m.output = m.output.value.replace("_sa_h2_" + str(round(hp - inc, 1)) + "_fc.xlsx", suffix)
...
def sensitivity_analysis_el(dict,Imin,fixed,summary_file_name,inc):
    ...
    for key in system_data.keys():
        ...
        for k in range(1,17):
            ...
            s_suffix = "_sa_el_" + str(round(sp,1))
...
def calc_fractions(m, n, summary):
    ...
    summary[32][n] = max(m.p_imp[t].value for t in m.T)
    ...
    if summary[26][n] != 0:
        summary[33][n] = summary[4][n] / (m.P_max[1] * summary[26][n]) #Load factor
...
def create_summary_matrix():
    ...
    summary = {1:case,2:p_dem,3:p_pv,4:p_fc,5:p_bio,6:p_import,7:q_dem,8:q_st,9:q_fc,10:q_bio, 11:q_dh,12:v_bat_ch,\
               ...
               29:Spot_mean,30:el_price,31:dh_price,32:p_imp_max,33:load_factor}
    return summary
...
november_peak = {1:"system_data_month_november.xls",2:"system_data_month_november_peak.xls",\
                 3:"system_data_month_november_peak_no_bat.xls"}
buildings = {1:"system_data_building_apartement.xls",2:"system_data_building_hostpital.xls",3:"system_data_building_nursing_home.xls",\
             4:"system_data_building_office.xls",5:"system_data_building_shop.xls",6:"system_data_building_university.xls",7:"system_data.xls"}
apartment = {1:"system_data_building_apartement.xls"}
hospital = {1:"system_data_building_hostpital.xls"}
nursing_home = {1:"system_data_building_nursing_home.xls"}
office = {1:"system_data_building_office.xls"}
shop = {1:"system_data_building_shop.xls"}
university = {1:"system_data_building_university.xls"}
...
# Building cases
#multiple_cases(year,False,True,"summary_year_250.xlsx")

#multiple_cases_strategic_fc_control(year_2050, False, False, "summary_year_2050_fc_control_jun_sep.xlsx")

#multiple_cases_base_case(apartment,"summary_building_apartment_base.xlsx")
#multiple_cases_base_case(hospital,"summary_building_hostpital_base.xlsx")
#multiple_cases_base_case(nursing_home,"summary_building_nursing_home_base.xlsx")
#multiple_cases_base_case(office,"summary_building_office_base.xlsx")
#multiple_cases_base_case(shop,"summary_building_shop_base.xlsx")
#multiple_cases_base_case(university,"summary_building_university_base.xlsx")

#multiple_cases(apartment,False,True,"summary_building_apartment_base.xlsx")
#multiple_cases(hospital,False,True,"summary_building_hostpital_base.xlsx")
#multiple_cases(nursing_home,False,True,"summary_building_nursing_home_base.xlsx")
#multiple_cases(office,False,True,"summary_building_office_base.xlsx")
#multiple_cases(shop,False,True,"summary_building_shop_base.xlsx")
#multiple_cases(university,False,True,"summary_building_university_base.xlsx")

#multiple_cases(summer_winter, True, False, "summary_summer_winter_200.xlsx")
#multiple_cases_blackout(summer_winter, "day",False,"summary_summer_winter_blackout_day_200.xlsx")
#multiple_cases_blackout(summer_winter, "night",False,"summary_summer_winter_blackout_night_200.xlsx")

"""Multiple weeks"""
# calc_dh_power_cost_old
#multiple_cases_weekly("system_data_2050.xls","summary_weekly_constant_200.xlsx",52)
...
#multiple_cases(year, True, False, "summary_year_200_x.xlsx")
#multiple_cases(year, False, True, "summary_year_forced_2030.xlsx")
#multiple_cases(year_2050, True, False, "summary_year_2050_200_x.xlsx")
#multiple_cases(year_2050, False, True, "summary_year_2050_forced_200.xlsx")


"""Sensitivity analysis"""
#sensitivity_analysis_h2(years, False, True, "summary_year_sa_h2_200.xlsx",0.1)
#sensitivity_analysis_el(years, False, True,"summary_year_sa_el_200.xlsx",0.1)
