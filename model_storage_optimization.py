...
def build_storage_model(system_data):
    ...
    v_bat_min = Storage["Max_dod"][1] * Cap_max_bat
    ...
    v_tes_min = Storage["Max_dod"][2] * Cap_max_tes
    ...
    m.C_g_fuel = Param(m.G, initialize=P_fuelc)      # Fuel cost per gen
    ...
    # Energy storage
    m.v_bat = Var(m.T, initialize=m.Min_v_bat, bounds=(m.Min_v_bat, np.Inf))
    m.p_bat_ch = Var(m.T, initialize=0, bounds=(0, P_bat_max))
    m.p_bat_dis = Var(m.T, initialize=0, bounds=(0, P_bat_max))
    m.v_bat_cap = Var(initialize=m.Min_cap_bat,bounds=(m.Min_cap_bat,m.Max_cap_bat))

    m.v_tes = Var(m.T, initialize=m.Min_v_tes, bounds=(m.Min_v_tes, np.Inf))
    m.q_tes_ch = Var(m.T, initialize=0, bounds=(0, Q_tes_max))
    m.q_tes_dis = Var(m.T, initialize=0, bounds=(0, Q_tes_max))
    m.v_tes_cap = Var(initialize=m.Min_cap_tes, bounds=(m.Min_cap_tes, m.Max_cap_tes))
    ...
    def max_bat(m, t):
        return m.v_bat[t] <= m.v_bat_cap
    m.c_bat_max = Constraint(m.T, rule=max_bat)
    ...
    def max_tes(m, t):
        return m.v_tes[t] <= m.v_tes_cap
    m.c_tes_max = Constraint(m.T, rule=max_tes)
    ...
    def OBJ_cost_min(model):
        ...
        import_costs = sum(m.p_imp[t] * m.C_imp[t] for t in m.T) + sum(m.dh[t] * m.C_dh for t in m.T)
        ...
        investment = sum(m.C_g_invest[g] for g in m.G) + sum(m.C_h_invest[h] for h in m.H) + m.C_pv_invest + m.C_st_invest
        storage_investment = m.v_bat_cap * m.C_bat_invest + m.v_tes_cap * m.v_tes_cap
        maintenance = sum(m.C_g_maint[g] for g in m.G) + sum(m.C_h_maint[h] for h in m.H) + m.C_pv_maint + m.C_st_maint
        ...
        return fuel_cost + import_costs + investment + maintenance + grid_power_costs + storage_investment -\
...
def calc_total_cost(m):
    ...
    fuel_cost = generator_operation + heat_operation
    import_costs = sum(m.p_imp[t].value * m.C_imp[t] for t in m.T) + sum(m.dh[t].value * m.C_dh_entar for t in m.T)
    ...
    certificate_savings = sum(m.C_g_fuel[2] * m.gen[2, t].value / m.P_eta[2] + m.pv[t] for t in m.T) * m.C_gc
    return fuel_cost + import_costs + investment + maintenance + grid_power_costs - export_costs - certificate_savings
